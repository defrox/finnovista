...
// Just activated the theme? - For later use
// function stag_just_activated(){
//   global $pagenow;
...
function stag_admin_footer(){
  echo '<span class="stag-credits">'.STAG_THEME_NAME.' v'.STAG_THEME_VERSION.'. Theme powered by <a href="//shop.codestag.com" target="_blank" rel="follow">Stag Framework v'.STAG_FRAMEWORK_VERSION.'</a></span>';
}
...
function stag_get_theme_changelog(){
  $stag_options = get_option('stag_framework_options');
  $log_url = STAG_UPDATE_URL.'/'.stag_to_slug($stag_options['theme_name']).'-changelog.xml';
  $transkey = 'stag_latest_theme_version_'.stag_to_slug($stag_options['theme_name']);
