<?php
add_action('widgets_init', create_function('', 'return register_widget("stag_section_blog_category2");'));

class stag_section_blog_category2 extends WP_Widget
{
    function stag_section_blog_category2()
    {
        $widget_ops = array('classname' => 'section-block', 'description' => __('Displays your recent blog post with extended features.', 'stag'));
        $control_ops = array('width' => 300, 'height' => 350, 'id_base' => 'stag_section_blog_category2');
        $this->WP_Widget('stag_section_blog_category2', __('Homepage: Blog Section Category 2', 'stag'), $widget_ops, $control_ops);
    }
    ...
    function widget($args, $instance)
    {
        ...
        $title = __($instance['title'], 'widget');
        $subtitle = __($instance['subtitle'], 'widget');
        $subtext = __($instance['subtext'], 'widget');
        ...
        <section id="<?php echo stag_to_slug($id); ?>" class="stag_section_blog_category2 videos section-block">
                    ...
                    <div class="video-item">
                        ...
                        if (sizeof($terms2) > 0 && is_array($terms2)) {
                            foreach ($terms2 as $term2) {
                                ...
                                // Color tag
                                if (in_array($category, $color_categories) &&
                                    substr($term2->description, 0, strlen("tagcolor=")) === "tagcolor=" &&
                                    !$setted2 && !$setted) {
                                    $pd_tag2 = $term2->name;
                                    $pd_css2 = $term2->slug;
                                    $pd_color2 = str_replace("tagcolor=", "", $term2->description);
                                    $setted2 = true;
                                }
                            }
                        }
                        ...
                        <div class="item">
                            ...
                            <div class="entry-content pdentry">
                                </strong><!-- Temporary Fix, somewhere is adding a strong tag. I guess it's a JS -->
                                <?php if ($pd_tag2 != ''): ?>
                                    <span><div class="pd-tag pd-tag-1 <?php echo $pd_css2; ?>" style="background-color: #<?php echo $pd_color2; ?>;"><?php echo $pd_tag2; ?></div></span>
                                <?php endif ?>
                                <?php if ($pd_tag != ''): ?>
                                    <span><div class="pd-tag pd-tag-2 <?php echo $pd_css; ?>" style="background-color: #<?php echo $pd_color; ?>;"><?php echo $pd_tag; ?></div></span>
                                <?php endif ?>
                                ...
                                <div class="boxed disqus" style="display: none;"><a href="<?php the_permalink(); ?>#disqus_thread"><i class="fa fa-comments fa-stack-2x fa-inverse"></i><s>0</s></a></div>
                            </div>
                        </div>
            <?php
            endwhile;
    ...
    function form($instance)
    {
        ...
        <p>
            <label for="<?php echo $this->get_field_id('subtext'); ?>"><?php _e('Sub Text:', 'stag'); ?></label>
            <input type="text" class="widefat" id="<?php echo $this->get_field_id('subtext'); ?>"
                   name="<?php echo $this->get_field_name('subtext'); ?>"
                   value="<?php echo @$instance['subtext']; ?>"/>
        </p>
        ...
        <p>
            ...
            <label for="<?php echo $this->get_field_id('alt_link'); ?>"><?php _e('Use alternative links:', 'stag'); ?></label>
            <input type="checkbox" class="checkbox" id="<?php echo $this->get_field_id('alt_links'); ?>"
