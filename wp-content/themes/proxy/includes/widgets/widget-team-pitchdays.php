<?php
add_action('widgets_init', create_function('', 'return register_widget("stag_section_team_pitchdays");'));

class stag_section_team_pitchdays extends WP_Widget
{
    function stag_section_team_pitchdays()
    {
        $widget_ops = array('classname' => 'section-block', 'description' => __('Displays your team members by category and pitchdays and their info.', 'stag'));
        $control_ops = array('width' => 300, 'height' => 350, 'id_base' => 'stag_section_team_pitchdays');
        $this->WP_Widget('stag_section_team_pitchdays', __('Homepage: Team by Category and Pitch Days Section', 'stag'), $widget_ops, $control_ops);
    }
        ...
        <section id="<?php echo stag_to_slug($id); ?>" class="section-block team-widget">
            ...
            if ($no_posts->publish > $number_posts): ?>
                ...
                <div <?php post_class($d_c); ?> >
                    <a class="bottom-link"
                       href="/team/?cat=<?php print urlencode($category) . ($_GET['lang'] != '' && $translate == 'on' ? '&lang=' . $_GET['lang'] : ''); ?>"><?php echo __('View all', 'stag') . ' ' . $title; ?>
                        &raquo;</a>
                </div>
            <?php
            endif;
