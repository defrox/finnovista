<?php
add_action('widgets_init', create_function('', 'return register_widget("stag_section_team_category");'));

class stag_section_team_category extends WP_Widget
{
    function stag_section_team_category()
    {
        $widget_ops = array('classname' => 'section-block', 'description' => __('Displays your team members by category and their info.', 'stag'));
        $control_ops = array('width' => 300, 'height' => 350, 'id_base' => 'stag_section_team_category');
        $this->WP_Widget('stag_section_team_category', __('Homepage: Team by Category Section', 'stag'), $widget_ops, $control_ops);
    }
    ...
    function widget($args, $instance)
    {
        ...
        $after_title = __($instance['after_title'], 'stag');
        ...
        <section id="<?php echo stag_to_slug($id); ?>" class="section-block team-widget">
                                        ...
                                        <div class="member-links">
                                            ...
                                            <?php if (get_post_meta(get_the_ID(), '_stag_team_url_linkedin', true) != ''): ?>
                                                <a href="<?php echo get_post_meta(get_the_ID(), '_stag_team_url_linkedin', true); ?>"
                                                   class="linkedin"></a>
                                            <?php endif ?>
                                            <?php //echo get_post_meta(get_the_ID(), '_stag_team_category', true); ?>
                                        </div>
                                    </div>
                                <?php endif; ?>
            ...
            if ($no_posts->publish > $number_posts && $more_link == 'true'): ?>
                ...
                <div <?php post_class($d_c); ?> >
                    <a class="bottom-link"
                       href="/team/?<?php print ($category != '' ? '&cat=' . urlencode($category) : '') . ($area != '' ? '&area=' . urlencode($area) : '') . ($fscity != '' ? '&fscity=' . urlencode($fscity) : '') . ($pdcity != '' ? '&pdcity=' . urlencode($pdcity) : '') . ($visacity != '' ? '&visacity=' . urlencode($visacity) : '') . ($_GET['lang'] != '' && $translate == 'on' ? '&lang=' . $_GET['lang'] : ''); ?>"><?php echo sprintf($view_all, $title); ?></a>
                </div>
            <?php
            endif;
            ?>
