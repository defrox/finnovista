...
if( $skinnyData['activatedplugin'] == 'wpsugarfree' ) {
	$Leads_setting_name = "Sugar CRM Settings";
}else
{
	$Leads_setting_name = "Suite CRM Settings";
}
...
<input type='hidden' id='revert_old_crm' value='wpsugarfree'>
<form id="smack-sugar-settings-form" action="<?php echo esc_url_raw($_SERVER['REQUEST_URI'] , $protocols=null ); ?>" method="post">
	...
	<div class="wp-common-crm-content" style="width: 100%;float: left;">
		...
		<label id="inneroptions" style="font-weight:bold;"> <?php echo $Leads_setting_name; ?></label>
		<table  class="settings-table">
			...
			<tr>
				...
				<td colspan="3">
					<input type='text' class='smack-sugar-pro-settings-text urlfield' name='url' id='smack_sugarfree_host_address' value="<?php echo sanitize_url( $config['url'] , $protocols=null ) ?>"/>
				</td>
			</tr>
			<tr>
				<td style='width:160px;padding-left:40px;'>
		...
		<p class="submit">
			<input type="submit" id="save_sugar_config" value="<?php _e('Save CRM Configuration');?>" class="button-primary" onclick="document.getElementById('loading-image').style.display = 'block'" style="float: right;"/>
...
jQuery("#save_sugar_config").click(function(){
	jQuery('#save_sugar_config').css('display', 'none');
	jQuery('#save_crm_config').css('display', '');
});

jQuery( "#save_sugar_config" ).click( function(){

var sugar_host_url = jQuery("#smack_sugarfree_host_address" ).val();
                if( sugar_host_url.match(/index.php/))
                {
                        ...
                        jQuery("#smack_sugarfree_host_address").val(sugar_replaced_host_url);
                }
