...
{
	include_once(SM_LB_PRO_DIR.'lib/vtwsclib/Vtiger/Net/HTTP_Client.php');
	include_once(SM_LB_PRO_DIR.'lib/vtwsclib/Vtiger/WSVersion.php');
	...
	class Vtiger_WSClient {
		...
		function doDelete($record) {
                        ...
                        $resultdata = $this->_client->doPost($getdata, true);
                        if($this->hasError($resultdata)) {
                                return false;
                        }
                        return $resultdata['result'];
