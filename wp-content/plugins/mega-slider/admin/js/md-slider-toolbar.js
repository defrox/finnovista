...
(function($) {
    var MdSliderToolbar = function(panel) {
        ...
        this.init = function() {
            ...
            $(document).on('click', ".panel-change-videothumb", function() {
                ...
                if(videoid != null) {
                    $.getJSON("http://gdata.youtube.com/feeds/api/videos/" + videoid[1] + "?v=2&alt=jsonc", function(data) {
                        if(data.data) {
                            ...
                            $("#videothumb").attr("src",info.thumbnail.hqDefault);
                        }
                    });
                } else {
                    ...
                    if (match != null) {
                        $.getJSON("http://vimeo.com/api/v2/video/" + match[1] + '.json?callback=?', {format: "json"}, function(data) {
                            if(data) {
                                ...
                                $("#videoname").val(data[0].title);
                                $("#videothumb").attr("src", data[0].thumbnail_medium);
                            }
                        });
                    }
                }
            ...
            $("#change-image").click(function() {
                ...
                wp.media.editor.send.attachment = function(props, attachment) {
                    $("textarea.mdt-imgalt", "#md-toolbar").val(attachment.title);
                    $("img.mdt-imgsrc", "#md-toolbar").attr("src", attachment.url);
                    $("input.mdt-fileid", "#md-toolbar").val(attachment.id);
                    self.panel.setImageData(attachment.id, attachment.title, attachment.url);
            ...
            $("#mdt-linkexpand a.mdt-link-remove").click(function() {
                self.panel.setItemLinkData(null);
        ...
        this.changeFontWeightOption = function(fontweight) {
            ...
            if(fontweight) {
                var fontweights = fontweight.toString().split(",");
		...
		this.changePositionLeft = function(left) {
            $("input.mdt-left", "#md-toolbar").val(Math.round(left));
        }
		this.changePositionTop = function(top) {
            $("input.mdt-top", "#md-toolbar").val(Math.round(top));
        }
