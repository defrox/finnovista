...
class AMPHTML_Sanitize {
	...
	public function sanitize_image( $img ) { //todo refactoring
		...
		$width = ( (int) $img->width > (int) $this->template->get_option( 'content_width' ) )
			? $this->template->get_option( 'content_width' ) : $img->width;
		...
		if ( ! empty( $img->outertext ) ) {
			...
			if ( $this->is_gif_url( $img->src ) ) {
				$this->template->add_embedded_element(
					array( 'slug' => 'amp-anim', 'src' => 'https://cdn.ampproject.org/v0/amp-anim-0.1.js' )
				);
				$img->tag = 'amp-anim';
			} else {
				$img->tag = 'amp-img';
	...
	public function sanitize_form( $form ) {
		...
		if ( $this->template->get_option( 'is_hidden_forms' ) ) {
			$form->outertext = "<p class='amp-no-form'>"
			                   . __( 'This page contains form, you can see it - ' )
			                   . "<a href='" . get_the_permalink() . "' target='_blank'>" . __( 'here' ) . "</a>.";
		} else {
			$this->template->add_embedded_element( array(
				'slug' => 'amp-form',
	...
	protected function sanitize_twitter( $el ) {
		$this->template->add_embedded_element(
			array( 'slug' => 'amp-twitter', 'src' => 'https://cdn.ampproject.org/v0/amp-twitter-0.1.js' )
		);
	...
	protected function sanitize_instagram( $el ) {
		$this->template->add_embedded_element(
			array( 'slug' => 'amp-instagram', 'src' => 'https://cdn.ampproject.org/v0/amp-instagram-0.1.js' )
		);

		foreach ( $el->find( 'a' ) as $link ) {
			$url_parts     = explode( '/', untrailingslashit( $link->href ) );
			$el->outertext = sprintf( "<amp-instagram width='1' height='1' layout='responsive' data-shortcode=%s></amp-instagram>", end( $url_parts ) );
		}
	}
	...
	public function sanitize_facebook($el, $src = false, $type = false) {
		// for iframe
		$this->template->add_embedded_element(
			array( 'slug' => 'amp-facebook', 'src' => 'https://cdn.ampproject.org/v0/amp-facebook-0.1.js' )
		);

		if( $src ) {
			if( $type == 'video' ) {
				$el->outertext = sprintf( "<amp-facebook height='400' width='400' layout='responsive' data-embed-as='video' data-href=%s></amp-instagram>", $src );
			} else {
				$el->outertext = sprintf( "<amp-facebook height='400' width='400' layout='responsive' data-href=%s></amp-instagram>", $src );
			}
			return;
		}
		...
		if( $class === "fb-post" ) {
			$link = $el->cite;
			$el->outertext = sprintf( "<amp-facebook height='400' width='400' layout='responsive' data-href=%s></amp-instagram>", $link );
		} elseif ( $class === "fb-video" ) {
			$anchors = $el->find( 'a' );
			$anchor = array_shift($anchors);
			$link = $anchor->href;
			$el->outertext = sprintf( "<amp-facebook height='400' width='400' layout='responsive' data-embed-as='video' data-href=%s></amp-instagram>", $link );
		}
	}
