...
class AMPHTML_Template extends AMPHTML_Template_Abstract {
	...
	public function get_element_fonts() {
		...
		return ".logo { Font-Family: {$this->fonts['logo_font']}, serif }"
		       ...
		       . ".amphtml-content, .breadcrumb li { Font-Family: {$this->fonts['content_font']}, serif }"
	...
	public function get_element_colors() {
		...
		return 'nav.amphtml-title,'
		       ...
		       . "nav.amphtml-title a, nav.amphtml-title div, div.main-navigation a, #amp-sidebar a, div.header button.amp-menu-sidebar { color: {$this->options->get( 'header_text_color' )} }"
		       . ".amphtml-add-to .a-button { background-color: {$add_to_cart_button}; border-color: {$add_to_cart_button} }"
		       . ".amphtml-add-to .a-button:hover, .amphtml-add-to .a-button:active { background-color: {$add_to_cart_button}; border-color: {$add_to_cart_button}; }"
	...
	public function set_archive_page_post( $id ) {
		...
		$this->content            = $this->get_content( $this->post );
		$this->featured_image     = $this->get_featured_image();
		$this->description        = ! empty( $this->post->post_excerpt ) ?
											$this->post->post_excerpt : $this->content;
	}
	...
	public function get_featured_image() {
		...
		if ( $post_thumbnail_id ) {
			$featured_image        = wp_get_attachment_image_src( $post_thumbnail_id, 'amphtml-image' );
