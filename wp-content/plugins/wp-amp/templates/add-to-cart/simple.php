...
if( 'add_to_cart_cart' == $this->get_template()->get_option( 'add_to_cart_behav' ) ) {
    global $woocommerce;
    $add_to_cart_url = $woocommerce->cart->get_cart_url();
    $add_to_cart_url = add_query_arg( 'add-to-cart', $product->id, $add_to_cart_url );
} else if ( 'add_to_cart' == $this->get_template()->get_option( 'add_to_cart_behav' ) ) {
    $add_to_cart_url = get_permalink( $product->id );
	$add_to_cart_url = add_query_arg( 'add-to-cart', $product->id, $add_to_cart_url );
} else {
    $add_to_cart_url = get_permalink( $product->id );
...
if ( $product->is_in_stock() ) : ?>
	...
	<p class="amphtml-add-to">
		<a class="a-button" href="<?php echo $add_to_cart_url ?>"><?php echo $this->get_template()->get_option( 'add_to_cart_text' ) ?></a>
	</p>
