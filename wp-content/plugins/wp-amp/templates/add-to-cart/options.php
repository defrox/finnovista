...
if( $add_to_cart_behav == 'add_to_cart_cart' ) {
    global $woocommerce;
    $form_action = $woocommerce->cart->get_cart_url();
} else {
    $form_action = get_permalink( $product->id );
}
...
if ( $product->is_in_stock() ) : ?>
	...
	<form action="<?php echo $form_action ?>" method="get" target="_blank" id="form1">
		<?php if ( $template->get_option( 'product_options' ) && $product->product_type === 'variable' ): ?>
			<div class="amp-product-options">
				...
				<div class="amp-atribute-select-block">
					...
					<select name="<?php echo wc_variation_attribute_name($attribute_name); ?>" class="amp-attribute-select">
						<?php foreach( $options as $option): ?>
							<option value="<?php echo $option; ?>"><?php echo esc_html( apply_filters( 'woocommerce_variation_option_name', $option ) );?></option>
						<?php endforeach; ?>
					</select>
				</div>
				<?php endforeach; ?>
		...
		<?php  if ( $add_to_cart_behav == 'add_to_cart' OR $add_to_cart_behav == 'add_to_cart_cart' ): ?>
			<input type="hidden" name="add-to-cart" value="<?php echo $product->id; ?>">
		<?php else : ?>
			<input type="hidden" name="add-to-cart-redirect" value="1">
		<?php endif; ?>
		<p class="amphtml-add-to">
			<input class="a-button" type="submit" value="<?php echo $this->get_template()->get_option( 'add_to_cart_text' ) ?>">
		</p>
	</form>
