...
if (!class_exists('SNP_NHP_Options'))
{
    ...
    {
	//define('SNP_NHP_OPTIONS_URL', site_url(str_replace(ABSPATH, '', trailingslashit(dirname(__FILE__)))));
...
//function
	...
	function _options_page()
	{
		...
		$this->page			 = add_submenu_page(
			$this->args['page_parent'], $this->args['page_title'], $this->args['menu_title'], $this->args['page_cap'], $this->args['page_slug'], array(&$this, '_options_page_html')
		);
	    }
	    else
	    {
		$this->page = add_menu_page(
			$this->args['page_title'], $this->args['menu_title'], $this->args['page_cap'], $this->args['page_slug'], array(&$this, '_options_page_html'), $this->args['menu_icon'], $this->args['page_position']
		);
	    }//else
...
//function	
	...
	{
	    global $post;
	    $ex = array('sendpulse_lists', 'salesmanago_lists', 'mailjet_lists', 'benchmarkemail_lists', 'sendgrid_lists', 'agilecrm_lists', 'elasticemail_lists', 'salesautopilot_lists' ,'myemma_lists','mailerlite_lists', 'activecampaign_fields','rocketresponder_lists', 'freshmail_lists', 'infusionsoft_lists','icontact_lists','mailchimp_lists','sendinblue_lists','sendpress_lists','egoi_lists','getresponse_lists','wysija_lists','campaignmonitor_lists','aweber_lists','mymail_lists','madmimi_lists','constantcontact_lists','aweber_auth');
		...
		if ($hook == 'snp_popups_page_snp_opt' || ($post && 'snp_popups' === $post->post_type))
		{
			...
			if (is_dir($this->dir . 'fields/' . $field_dir) && is_readable($this->dir . 'fields/' . $field_dir))
			{
			    ...
			    if (class_exists($field_class) && method_exists($field_class, 'enqueue'))// && $field_class=='SNP_NHP_Options_builder')
			    {
	...
	function _register_setting()
	{
		...
		if (isset($section['fields']))
		{
			...
			add_settings_field($fieldk . '_field', $th, array(&$this, '_field_input'), $k . '_snp_section_group', $k . '_snp_section', $field); // checkbox
		    }//foreach
		}//if(isset($section['fields'])){
	    }//foreach
	...
	function _set_custom_fields()
	{
		...
		foreach ((array) $this->customfields as $key => $c)
		{
		    ...
		    {
			add_meta_box($c['id'], $c['title'], array(&$this, '_custom_fields_html'), $post_type, $c['context'], $c['priority'], array('id'	 => $c['id'], 'key'	 => $key));
		    }
		}
	    }
	}
	...
	function _save_custom_fields_postdata($post_id)
	{
		...
		elseif ($data != get_post_meta($post_id, $this->args['opt_name'] . '_' . $k, true))
		{
