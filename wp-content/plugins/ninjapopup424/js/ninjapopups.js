...
function snp_onclose_popup()
{
    ...
    if (!jQuery.cookie(snp_cookie_prefix + 'snp_' + popup))
    {
        if (!cookie_close)
        {
            cookie_close = -1;
...
function snp_open_popup(href, target, popup, type)
{
    ...
    {
        if (jQuery.cookie(snp_cookie_prefix + 'snp_' + popup) == 1) {
            return true;
        }
    }
    ...
    if (jQuery('#snppopup-exit').length > 0)
    {
        ...
        if (snp_show_on_exit == 2)
        {
            ...
            jQuery(window).bind('beforeunload', function (e) {
                ...
                if (e) {
                    e.returnValue = jQuery('#snppopup-exit .snp_exit_js_alert_text').val();
                }
                return jQuery('#snppopup-exit .snp_exit_js_alert_text').val();
            });
        }
